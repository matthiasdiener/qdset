...
void _PyDict_ClearFreeList(PyInterpreterState *interp)
{
#if PyDict_MAXFREELIST > 0
...
void _PyDict_Fini(PyInterpreterState *interp)
{
...
int _PyDict_CheckConsistency(PyObject *op, int check_content)
{
#define CHECK(expr)                                              \
    do                                                           \
    {                                                            \
...
free_keys_object(PyDictKeysObject *keys)
{
    ...
    assert(state->keys_numfree != -1);
#endif
    if (DK_LOG_SIZE(keys) == PyDict_LOG_MINSIZE && state->keys_numfree < PyDict_MAXFREELIST && DK_IS_UNICODE(keys))
    {
...
dictresize(PyDictObject *mp, uint8_t log2_newsize, int unicode)
{
    ...
    { // oldkeys is combined.
        ...
        { // oldkeys is combined unicode
            PyDictUnicodeEntry *oldentries = DK_UNICODE_ENTRIES(oldkeys);
            if (unicode)
            { // combined unicode -> combined unicode
                PyDictUnicodeEntry *newentries = DK_UNICODE_ENTRIES(mp->ma_keys);
...
int _PyDict_SetItem_KnownHash(PyObject *op, PyObject *key, PyObject *value,
                              Py_hash_t hash)
{
    PyDictObject *mp;
 ...
 */
int _PyDict_Next(PyObject *op, Py_ssize_t *ppos, PyObject **pkey,
...
_PyDict_KeysSize(PyDictKeysObject *keys)
{
    ...
    return (sizeof(PyDictKeysObject) + ((size_t)1 << keys->dk_log2_index_bytes) + USABLE_FRACTION(DK_SIZE(keys)) * es);
}
...
static PyMethodDef mapp_methods[] = {
    DICT___CONTAINS___METHODDEF{"__getitem__", _PyCFunction_CAST(dict_subscript), METH_O | METH_COEXIST,
                                getitem__doc__},
    ...
    DICT_GET_METHODDEF
        DICT_SETDEFAULT_METHODDEF
            DICT_POP_METHODDEF
                DICT_POPITEM_METHODDEF{"keys", dictkeys_new, METH_NOARGS,
                                       keys__doc__},
    ...
    {"update", _PyCFunction_CAST(dict_update), METH_VARARGS | METH_KEYWORDS,
     update__doc__},
    DICT_FROMKEYS_METHODDEF{"clear", (PyCFunction)dict_clear, METH_NOARGS,
                            clear__doc__},
    {"copy", (PyCFunction)dict_copy, METH_NOARGS,
     copy__doc__},
    DICT___REVERSED___METHODDEF{"__class_getitem__", Py_GenericAlias, METH_O | METH_CLASS, PyDoc_STR("See PEP 585")},
    {NULL, NULL} /* sentinel */
};
...
int _PyDict_Contains_KnownHash(PyObject *op, PyObject *key, Py_hash_t hash)
{
    PyDictObject *mp = (PyDictObject *)op;
    PyObject *value;
    Py_ssize_t ix;
...
int _PyDict_ContainsId(PyObject *op, _Py_Identifier *key)
{
...
int _PyDict_SetItemId(PyObject *v, _Py_Identifier *key, PyObject *item)
{
    PyObject *kv;
    kv = _PyUnicode_FromId(key); /* borrowed */
    if (kv == NULL)
...
{
    assert(view != NULL);
    assert(PyDictKeys_Check(view) || PyDictValues_Check(view) || PyDictItems_Check(view));
...
int _PyObject_InitializeDict(PyObject *obj)
{
...
int _PyObject_StoreInstanceAttribute(PyObject *obj, PyDictValues *values,
...
int _PyObject_IsInstanceDictEmpty(PyObject *obj)
{
    PyTypeObject *tp = Py_TYPE(obj);
    if (tp->tp_dictoffset == 0)
    {
...
int _PyObjectDict_SetItem(PyTypeObject *tp, PyObject **dictptr,
