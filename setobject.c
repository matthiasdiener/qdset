...
static qdsetentry *
set_lookkey(QdPySetObject *so, PyObject *key, Py_hash_t hash)
{
    qdsetentry *table;
    qdsetentry *entry;
    size_t perturb = hash;
...
static int set_table_resize(QdPySetObject *, Py_ssize_t);

static int
set_add_entry(QdPySetObject *so, PyObject *key, Py_hash_t hash)
{
    qdsetentry *table;
    qdsetentry *freeslot;
    qdsetentry *entry;
...
*/
static void
set_insert_clean(qdsetentry *table, size_t mask, PyObject *key, Py_hash_t hash)
{
    qdsetentry *entry;
    size_t perturb = hash;
...
static int
set_contains_entry(QdPySetObject *so, PyObject *key, Py_hash_t hash)
{
    qdsetentry *entry;
...
static int
set_discard_entry(QdPySetObject *so, PyObject *key, Py_hash_t hash)
{
    qdsetentry *entry;
    PyObject *old_key;
...
static int
set_add_key(QdPySetObject *so, PyObject *key)
{
    Py_hash_t hash;
...
static int
set_contains_key(QdPySetObject *so, PyObject *key)
{
    Py_hash_t hash;
...
static int
set_discard_key(QdPySetObject *so, PyObject *key)
{
    Py_hash_t hash;
...
static int
set_clear_internal(QdPySetObject *so)
{
    qdsetentry *entry;
 ...
 */
static int
set_next(QdPySetObject *so, Py_ssize_t *pos_ptr, qdsetentry **entry_ptr)
{
    Py_ssize_t i;
    Py_ssize_t mask;
    qdsetentry *entry;
...
set_len(PyObject *so)
{
    return ((QdPySetObject *)so)->used;
}

static int
set_merge(QdPySetObject *so, PyObject *otherset)
{
    QdPySetObject *other;
    PyObject *key;
    Py_ssize_t i;
    qdsetentry *so_entry;
    qdsetentry *other_entry;

    assert(PyAnySet_Check(so));
    assert(PyAnySet_Check(otherset));

    other = (QdPySetObject *)otherset;
...
static int
set_traverse(QdPySetObject *so, visitproc visit, void *arg)
{
    Py_ssize_t pos = 0;
    qdsetentry *entry;
...
{
    printf("hash\n");
    QdPySetObject *so = (QdPySetObject *)self;
    Py_uhash_t hash = 0;
    qdsetentry *entry;
...
{
    PyObject_HEAD
        QdPySetObject *si_set; /* Set to NULL when iterator is exhausted */
    Py_ssize_t si_used;
    Py_ssize_t si_pos;
    Py_ssize_t len;
} setiterobject;
...
static int
set_update_internal(QdPySetObject *so, PyObject *other)
{
    PyObject *key, *it;
...
static PyObject *
set_update(QdPySetObject *so, PyObject *args)
{
    Py_ssize_t i;
...
make_new_set(PyTypeObject *type, PyObject *iterable)
{
    ...
    so = (QdPySetObject *)type->tp_alloc(type, 0);
    if (so == NULL)
        return NULL;
...
static void
set_swap_bodies(QdPySetObject *a, QdPySetObject *b)
{
    Py_ssize_t t;
    qdsetentry *u;
...
static PyObject *
set_copy(QdPySetObject *so, PyObject *Py_UNUSED(ignored))
{
    return make_new_set_basetype(Py_TYPE(so), (PyObject *)so);
}

static PyObject *
frozenset_copy(QdPySetObject *so, PyObject *Py_UNUSED(ignored))
{
...
static PyObject *
set_clear(QdPySetObject *so, PyObject *Py_UNUSED(ignored))
{
...
static PyObject *
set_union(QdPySetObject *so, PyObject *args)
{
    QdPySetObject *result;
    PyObject *other;
    Py_ssize_t i;

    result = (QdPySetObject *)set_copy(so, NULL);
    if (result == NULL)
        return NULL;
...
static PyObject *
set_or(QdPySetObject *so, PyObject *other)
{
    QdPySetObject *result;
...
static PyObject *
set_ior(QdPySetObject *so, PyObject *other)
{
    if (!PyAnySet_Check(other))
        Py_RETURN_NOTIMPLEMENTED;
...
static PyObject *
set_intersection(QdPySetObject *so, PyObject *other)
{
    QdPySetObject *result;
    PyObject *key, *it, *tmp;
    Py_hash_t hash;
    ...
    result = (QdPySetObject *)make_new_set_basetype(Py_TYPE(so), NULL);
    if (result == NULL)
        return NULL;
    ...
    if (PyAnySet_Check(other))
    {
        ...
        {
            tmp = (PyObject *)so;
            so = (QdPySetObject *)other;
            other = tmp;
        }

        while (set_next((QdPySetObject *)other, &pos, &entry))
        {
            key = entry->key;
            hash = entry->hash;
            Py_INCREF(key);
...
static PyObject *
set_intersection_multi(QdPySetObject *so, PyObject *args)
{
    Py_ssize_t i;
    ...
    {
        PyObject *other = PyTuple_GET_ITEM(args, i);
        PyObject *newresult = set_intersection((QdPySetObject *)result, other);
        if (newresult == NULL)
        {
            Py_DECREF(result);
            return NULL;
...
static PyObject *
set_intersection_update(QdPySetObject *so, PyObject *other)
{
    PyObject *tmp;
    ...
    if (tmp == NULL)
        return NULL;
    set_swap_bodies(so, (QdPySetObject *)tmp);
    Py_DECREF(tmp);
    Py_RETURN_NONE;
}

static PyObject *
set_intersection_update_multi(QdPySetObject *so, PyObject *args)
{
    PyObject *tmp;
...
static PyObject *
set_and(QdPySetObject *so, PyObject *other)
{
    if (!PyAnySet_Check(so) || !PyAnySet_Check(other))
        Py_RETURN_NOTIMPLEMENTED;
...
static PyObject *
set_iand(QdPySetObject *so, PyObject *other)
{
    PyObject *result;
...
static PyObject *
set_isdisjoint(QdPySetObject *so, PyObject *other)
{
    PyObject *key, *it, *tmp;
    int rv;
...
static int
set_difference_update_internal(QdPySetObject *so, PyObject *other)
{
    if ((PyObject *)so == other)
...
static PyObject *
set_difference_update(QdPySetObject *so, PyObject *args)
{
    Py_ssize_t i;
...
static PyObject *
set_copy_and_difference(QdPySetObject *so, PyObject *other)
{
    PyObject *result;
    ...
    if (result == NULL)
        return NULL;
    if (set_difference_update_internal((QdPySetObject *)result, other) == 0)
        return result;
    Py_DECREF(result);
    return NULL;
}
...
static PyObject *
set_difference(QdPySetObject *so, PyObject *other)
{
    PyObject *result;
    PyObject *key;
    Py_hash_t hash;
    ...
    if (PyDict_CheckExact(other))
    {
        while (set_next(so, &pos, &entry))
        {
            ...
            if (!rv)
            {
                if (set_add_entry((QdPySetObject *)result, key, hash))
                {
                    Py_DECREF(result);
                    Py_DECREF(key);
                    return NULL;
    ...
    while (set_next(so, &pos, &entry))
    {
        ...
        hash = entry->hash;
        Py_INCREF(key);
        rv = set_contains_entry((QdPySetObject *)other, key, hash);
        if (rv < 0)
        {
            Py_DECREF(result);
            Py_DECREF(key);
...
static PyObject *
set_difference_multi(QdPySetObject *so, PyObject *args)
{
    Py_ssize_t i;
    ...
    {
        other = PyTuple_GET_ITEM(args, i);
        if (set_difference_update_internal((QdPySetObject *)result, other))
        {
            Py_DECREF(result);
            return NULL;
        }
...
static PyObject *
set_sub(QdPySetObject *so, PyObject *other)
{
    if (!PyAnySet_Check(so) || !PyAnySet_Check(other))
        Py_RETURN_NOTIMPLEMENTED;
...
static PyObject *
set_isub(QdPySetObject *so, PyObject *other)
{
    if (!PyAnySet_Check(other))
        Py_RETURN_NOTIMPLEMENTED;
    if (set_difference_update_internal(so, other))
...
static PyObject *
set_symmetric_difference_update(QdPySetObject *so, PyObject *other)
{
    QdPySetObject *otherset;
    PyObject *key;
    Py_ssize_t pos = 0;
    Py_hash_t hash;
    qdsetentry *entry;
    ...
    {
        Py_INCREF(other);
        otherset = (QdPySetObject *)other;
    }
    else
    {
        otherset = (QdPySetObject *)make_new_set_basetype(Py_TYPE(so), other);
        if (otherset == NULL)
            return NULL;
    }
...
static PyObject *
set_symmetric_difference(QdPySetObject *so, PyObject *other)
{
    PyObject *rv;
    QdPySetObject *otherset;
...
static PyObject *
set_xor(QdPySetObject *so, PyObject *other)
{
    if (!PyAnySet_Check(so) || !PyAnySet_Check(other))
        Py_RETURN_NOTIMPLEMENTED;
...
static PyObject *
set_ixor(QdPySetObject *so, PyObject *other)
{
    PyObject *result;
...
static PyObject *
set_issubset(QdPySetObject *so, PyObject *other)
{
    qdsetentry *entry;
    Py_ssize_t pos = 0;
    int rv;
    ...
    while (set_next(so, &pos, &entry))
    {
        PyObject *key = entry->key;
        Py_INCREF(key);
        rv = set_contains_entry((QdPySetObject *)other, key, entry->hash);
        Py_DECREF(key);
        if (rv < 0)
        {
            return NULL;
...
static PyObject *
set_issuperset(QdPySetObject *so, PyObject *other)
{
    if (PyAnySet_Check(other))
    {
        return set_issubset((QdPySetObject *)other, (PyObject *)so);
    }
...
static PyObject *
set_richcompare(QdPySetObject *v, PyObject *w, int op)
{
    PyObject *r1;
    int r2;
    ...
    case Py_EQ:
        if (PySet_GET_SIZE(v) != PySet_GET_SIZE(w))
            Py_RETURN_FALSE;
        if (v->hash != -1 &&
            ((QdPySetObject *)w)->hash != -1 &&
            v->hash != ((QdPySetObject *)w)->hash)
            Py_RETURN_FALSE;
...
static PyObject *
set_add(QdPySetObject *so, PyObject *key)
{
    if (set_add_key(so, key))
        return NULL;
    Py_RETURN_NONE;
...
static int
set_contains(QdPySetObject *so, PyObject *key)
{
    PyObject *tmpkey;
    int rv;
...
static PyObject *
set_direct_contains(QdPySetObject *so, PyObject *key)
{
    long result;
...
static PyObject *
set_remove(QdPySetObject *so, PyObject *key)
{
    PyObject *tmpkey;
    int rv;
...
static PyObject *
set_discard(QdPySetObject *so, PyObject *key)
{
    PyObject *tmpkey;
    int rv;
...
static PyObject *
set_reduce(QdPySetObject *so, PyObject *Py_UNUSED(ignored))
{
...
static PyObject *
set_sizeof(QdPySetObject *so, PyObject *Py_UNUSED(ignored))
{
    Py_ssize_t res;
...
static int
set_init(QdPySetObject *self, PyObject *args, PyObject *kwds)
{
    PyObject *iterable = NULL;
...
#ifdef Py_DEBUG
static PyObject *test_c_api(QdPySetObject *so, PyObject *Py_UNUSED(ignored));
...
int PySet_Clear(PyObject *set)
{
    ...
    return set_clear_internal((QdPySetObject *)set);
}
...
int PySet_Contains(PyObject *anyset, PyObject *key)
{
    ...
    return set_contains_key((QdPySetObject *)anyset, key);
}
...
int PySet_Discard(PyObject *set, PyObject *key)
{
    ...
    return set_discard_key((QdPySetObject *)set, key);
}
...
int PySet_Add(PyObject *anyset, PyObject *key)
{
    ...
    return set_add_key((QdPySetObject *)anyset, key);
}
...
int _PySet_NextEntry(PyObject *set, Py_ssize_t *pos, PyObject **key, Py_hash_t *hash)
{
    ...
    if (set_next((QdPySetObject *)set, pos, &entry) == 0)
        return 0;
    *key = entry->key;
    *hash = entry->hash;
    return 1;
...
PySet_Pop(PyObject *set)
{
    ...
    return set_pop((QdPySetObject *)set, NULL);
}
...
int _PySet_Update(PyObject *set, PyObject *iterable)
{
    ...
    return set_update_internal((QdPySetObject *)set, iterable);
}
...
static PyObject *
test_c_api(QdPySetObject *so, PyObject *Py_UNUSED(ignored))
{
    Py_ssize_t count;
    const char *s;
    Py_ssize_t i;
...
static PyObject _dummy_struct = {
    _PyObject_EXTRA_INIT 2, &_PySetDummy_Type};
